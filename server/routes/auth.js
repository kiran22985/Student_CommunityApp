...
authRouter.post("/custlogin", async (req, res) => {
  try {
    const { email, password } = req.body;

    const user = await custModel.findOne({ email });
    if (!user) {
      return res
        .status(400)
...
authRouter.post("/tokenIsValid", async (req, res) => {
  try {
    ...
    const user = await custModel.findById(verified.id);
...
//   const user = await custModel.findById(req.user);
